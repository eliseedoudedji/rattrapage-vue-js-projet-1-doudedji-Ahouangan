...
function login() {
  user.value = {
    ...
    userPassWord: thePassWord.value
  }
  // console.log(user.value)
  userStore.toLogin(user.value)
  if (userStore.canConnect == true) {
    console.log('vous pouvez vous connecté')
    router.push('/taches')
  } else {
    console.log('Pas possible')
    seConnecter.value = "Vous n'avez pas de compte"
  }
}
</script>
<template>
    ...
    <div class="font-[sans-serif] max-w-4xl flex items-center mx-auto md:h-screen p-4">
      ...
      >
        ...
        <form class="md:col-span-2 w-full py-6 px-6 sm:px-16">
          <div class="mb-6">
            <h3 class="text-white text-2xl font-bold">Se connecter</h3>
          </div>
          <span style="color: red">{{ seConnecter }}</span>
          <div class="space-y-6">
            <div>
          ...
          <p class="text-sm mt-6 text-center">
            ...
            <RouterLink to="/register" class="text-blue-600 font-semibold hover:underline ml-1">
              S'enregistrer
            </RouterLink>
          </p>
        </form>
